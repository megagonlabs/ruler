...
class AnnotationDisplayCollapse extends React.Component {
    ...
    collapseText(text, MAX_COLLAPSED_MARGIN = 100) {
        const max_start = this.props.annotations[0]
            ? this.props.annotations[0].start_offset
            : 0;
        const min_end = this.props.annotations[0]
            ? this.props.annotations[0].end_offset
            : 0;
    ...
    render() {
        ...
        return (
            <Grid container item direction="row" xs={12} justify="space-around">
                <Grid item xs={11}>
                    <AnnotationDisplay
