...
class LRStatisticsPane extends React.Component {
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      Object.keys(prevProps.statistics).length > 0
    ) {
      ...
      if (delta > 0.0) {
        cellContent = (
          <span style={{ color: "green" }}>
            <ArrowDropUpIcon /> {style(delta)}{" "}
          </span>
        );
      } else if (delta < -0.0) {
        cellContent = (
          <span style={{ color: "red" }}>
            <ArrowDropDownIcon /> {style(delta)}
          </span>
        );
      }
    ...
    return (
      <Paper className={classes.paper}>
        <Typography className={classes.title} variant="h6" id="tableTitle">
          <RefreshIcon
            onClick={this.props.getLRStatistics}
            disabled={this.props.pending}
          ></RefreshIcon>
          Trained Model Statistics
        </Typography>
        <Typography variant="body1">
          Train a logistic regression model with bag-of-words features on your
          training set.
        </Typography>
        ...
        <Table
          ...
          size="small"
          aria-label="labeling statistics"
        >
          <TableBody>
                ...
                return (
                  <TableRow key={key}>
                    ...
                    <TableCell align="right">
                      {style(this.props.statistics[key])}
                    </TableCell>
                    {this.statDelta(key)}
                  </TableRow>
                );
        ...
        <Table>
          <TableBody>
            <TableRow key="class_headers">
              <TableCell>
                <Typography variant="h6">Class0</Typography>
              </TableCell>
              <TableCell />
              {prevStats ? <TableCell /> : null}
              <TableCell>
                <Typography variant="h6">Class1</Typography>
              </TableCell>
              <TableCell />
            </TableRow>
              ...
              if (key + "_0" in this.props.statistics) {
                return (
                  <TableRow key={key}>
                    ...
                    <TableCell align="right">
                      {style(this.props.statistics[key + "_0"])}
                    </TableCell>
                    {this.statDelta(key + "_0")}
                    <TableCell>{key + "_1"}</TableCell>
                    <TableCell align="right">
                      {style(this.props.statistics[key + "_1"])}
                    </TableCell>
